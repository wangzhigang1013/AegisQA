...
class JsonDocumentRepository:
    ...
    def __init__(self, store: DocumentStore, collection: str, id_key: str, filename_factory: FilenameFactory = str) -> None:
        self.store = store
...
class AuditEventRepository:
    ...
    def __init__(self, store: DocumentStore) -> None:
        self.store = store
        self.stream_parts: tuple[str, str] = ("audit", "events.jsonl")
...
class TaskRepository(JsonDocumentRepository):
    ...
    从全部记录缩小到该 run_id 对应的单条任务，行为与全量扫描完全一致。
    ...
    def __init__(self, store: DocumentStore) -> None:
        ...
        self._run_id_index: dict[str, list[dict[str, Any]]] | None = None

    def save(self, record: dict[str, Any]) -> dict[str, Any]:
        result = super().save(record)
        self._run_id_index = None
        return result
    ...
    def _ensure_index(self) -> dict[str, list[dict[str, Any]]]:
        if self._run_id_index is None:
            ...
            self._run_id_index = index
        return self._run_id_index
