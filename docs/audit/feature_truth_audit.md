...
| Trace | Trace Tree/Flow、Step 详情与调试抽屉 | `/tasks/{task_id}/trace-*`、`/runs/{run_id}/items/{item_id}/steps/{step_id}/replay|prompt-debug|repro-bundle` | Run item/step | 是，来自 RunItemStep | 不直接阻断，用于定位和 Replay | 后端、前端、E2E | 旧 Run 缺字段时降级展示；Replay 默认不隐式调用外部模型 | 保留主流程 | 补浏览器级 E2E 覆盖 Step 调试抽屉 |
...
| CI Gate | feature flag/治理入口 | Productization CI Gate API | Gate configs/history | 是，基于任务指标 | 是，已验证低通过率阻断 | 后端、前端、E2E | 高级治理能力，默认应受 flag 控制 | 保留但不夸大生产 | 生产 smoke 中加入失败阻断样例 |
| Experiment | 高级页面 | Productization API | Experiment snapshots | 部分，依赖来源任务 | 不作为主阻断 | 后端、前端 | 试用级能力 | 保留为高级模块 | 明确 flag 与页面 disabled 状态 |
...
| Candidate Assets | 高级页面 | Productization API | Candidate records | 部分，依赖候选来源 | 可受 CI Gate 阻断 | 后端、前端 | 试用级能力 | 默认不进主流程 | 发布前保持 flag/disabled 口径 |
| Annotation | Annotation Queue | Productization API | Annotation tasks | 是，来自任务样本 | 不阻断执行 | 后端、前端、E2E | 人审回流是试用级闭环 | 保留为高级模块 | 增加审核结果回流 Golden 的审计证据 |
| Judge | Judge Audit 页面 | Judge/Profile API | Judge audit/trends | 部分，真实数据可接入，趋势可计算 | 不直接阻断主任务 | 后端、前端 | mock provider 可用于离线 | 保留为治理能力 | 真实 provider 下验证 judge prompt trace |
...
3. 完善 Replay、Prompt Debug、Repro Bundle 的任务级浏览器 E2E。
4. 将 feature flag、disabled 页面和主导航收口规则做成固定测试。
