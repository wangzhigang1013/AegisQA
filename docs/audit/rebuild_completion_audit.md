...
| ArtifactStore | 新增 `ArtifactStore` / `LocalArtifactStore`，本地支持产物写入、读取、元数据、路径安全和大小限制；Dataset 上传/物化已接入 `uploaded_datasets`，Step Repro Bundle 已接入 `repro_bundles`，Skill Package 原始 zip 已接入 `skill_packages`，Task Report 导出已接入 `reports` | 完成最小接口和四类主产物接入 |
...
| ArtifactStore 接入 | 需要 Prompt/LLM 原始响应等运行时产物逐步写入 ArtifactStore | Dataset 上传/物化、Repro Bundle、Skill Package 原始 zip 和 Task Report 导出已接入；rendered prompts/raw llm responses 仍主要在运行记录内 | 按资产类型继续接入 ArtifactStore |
