...
| Replay/Repro | 后端 Step Replay、Prompt Debug、Repro Bundle 已可调用；前端抽屉深交互仍待产品化 | 已有 API 和定向测试，缺浏览器级操作验证 | 补前端抽屉按钮调用和 E2E |
...
| Replay/Debug/Repro | 后端试用级完成 | Step 级 Replay、Prompt Debug、Repro Bundle 已有 API；前端深交互仍需补 E2E |
...
3. Trace Flow Step 抽屉可以直接调用 Replay、Prompt Debug、Repro Bundle，并有浏览器级 E2E 覆盖。
