...
体验效率优化批次已完成实现与主要验证：后端新增真实工作台聚合、任务动作摘要、Trace Step 诊断和 Report 推荐动作；前端 Overview、Task 操作抽屉、Trace Flow、Report/Badcase 入口已改成围绕“下一步可执行动作”和真实证据展示，生产路径不引入 demo 兜底。后端全量测试、前端类型检查和前端全量 Vitest 已通过；本批次未重新执行前端生产构建和 Playwright E2E。
...
### 2026-06-05 体验效率优化工作台与诊断闭环

- 改动摘要：按当前项目进度落地第一批体验效率优化，把用户最常用的 Overview、Task、Trace Flow、Report/Badcase 路径从“分散查看数据”推进到“看到真实状态、失败原因和下一步动作”。后端新增轻量体验聚合层，前端主页面改用真实工作台 API 和后端动作契约，避免依赖 demo 数据或前端猜测。
- 主要能力：
  - `GET /overview/workbench` 返回真实工作台摘要、继续处理列表、Gate 风险、最近报告和最近任务，不再让概览页拉取完整 Run 列表。
  - `GET /tasks/{task_id}` 增加 `latest_run_summary`、`preflight_summary`、`gate_summary`、`available_actions`，Task 操作抽屉优先使用后端动作可用性和禁用原因。
  - Trace Flow Step 增加 `resolved_input`、`raw_output`、`validated_output`、`schema_errors`、`prompt_calls`、`diagnostic_tags`、`error_explanation`、`available_actions`，失败定位从旧 snapshot 升级为可诊断视图。
  - Task Report 增加 `primary_findings`、`recommended_actions`、`action_targets`，报告页把优先结论、Trace 定位、修复任务生成和 Badcase 批量修复入口聚合到同一工作流。
  - 测试 fixture 迁移到 `frontend/src/test/fixtures/demo.ts`，避免落入被 `.gitignore` 忽略的 `frontend/src/data/` 路径。
  - Windows 全量 Vitest 下 `lazyCharts`、`lazyRoutes` 两个慢测试显式设置 60 秒超时，降低并行运行时的假超时风险。
- 变更文件：
  - `aegisqa/api/experience.py`
  - `aegisqa/api/routes/governance.py`
  - `aegisqa/api/routes/tasks.py`
  - `aegisqa/reports/trace_flow.py`
  - `tests/test_experience_efficiency.py`
  ...
  - `frontend/src/test/fixtures/demo.ts`
...
- 验证命令：
  - `python -m pytest tests/test_experience_efficiency.py -q`
  - `python -m pytest -q`
...
- 测试结果：
  - 体验效率后端目标测试：4 passed。
  - 后端全量测试：通过，仅保留既有 Starlette / Windows cache warning。
  - 前端类型检查：通过。
  - 前端全量 Vitest：12 个测试文件、133 passed。
  - `git diff --check`：通过，仅有 Windows LF/CRLF 提示。
  - 本批次未重新执行 `cd frontend && npm run build` 和 `cd frontend && npm run e2e`；如进入发布前验收，需要补跑这两项。
- 下一步：
  - P1：补一个浏览器级冒烟，覆盖 Overview 工作台、Task 操作抽屉、Trace Step 详情、Report 推荐动作到 Badcase 修复入口的主链路。
  - P1：把后端 `available_actions` 扩展为更统一的动作 schema，减少 Overview、Task、Trace、Report 间的字段差异。
  - P2：把 Report 推荐动作与 Repair Task 动作定义抽为共享契约，继续降低前后端字符串漂移风险。
