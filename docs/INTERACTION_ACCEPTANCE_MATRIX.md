...
| 候选资产中心 | 负责人工作量与批量治理 | 可用，展示总候选、待处理、逾期、已升级、未指派和负责人维度工作量；支持在页面编辑批量指派负责人和 `max_open_per_owner` 容量限制，再把当前列表批量指派给负责人、批量审批当前列表、升级逾期候选，并在表格中展示负责人、截止时间、逾期和升级状态；超容量候选会跳过并给出容量跳过反馈 | `GET /prompt-skill-candidates/workload`、`POST /prompt-skill-candidates/bulk-assign`、`POST /prompt-skill-candidates/bulk-review`、`POST /prompt-skill-candidates/escalate-overdue` | `python -m pytest tests\test_task_flow_optimization.py -q -k "bulk_governance"` 覆盖批量指派、工作量聚合、逾期升级和批量审批；`python -m pytest tests\test_task_flow_optimization.py -q -k "owner_capacity"` 覆盖负责人容量限制和 skipped 明细；`npm test -- src/test/App.test.tsx -t "候选资产中心支持审批"` 覆盖负责人工作量、批量按钮、可编辑负责人、可编辑容量上限、请求体和容量跳过反馈 |
