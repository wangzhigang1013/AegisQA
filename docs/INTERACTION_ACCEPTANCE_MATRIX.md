...
| 执行中心 | 创建任务 | 可用，独立向导选择 Dataset Version 和 Workflow Version；未选择时禁用创建；可选择执行参数模板，一键填充评测目的、质量门槛、并发、repeat、重试和成本预算；必须先运行与当前 Dataset、Workflow、执行模板、评测目的、质量门槛、repeat 和成本预算匹配的 Preflight，任一关键参数变化后会提示重跑并禁用创建；Preflight passed/warning 才能直接创建，blocked 时默认禁用创建，必须勾选风险确认才会携带 `allow_blocked_preflight=true` 创建；后端永远重算 Preflight 作为事实源，客户端伪造 passed 不能绕过真实阻断；后端也会拒绝与当前请求不匹配的旧 `preflight_result` 并返回 `TASK_PREFLIGHT_STALE`；重跑后如果 Preflight 不再 blocked，会自动清除旧强制创建标记；支持评测目的、质量门槛、Preflight 检查表、分片大小、并发、repeat、最大重试、重试退避、成本预算，并保存到任务快照 | `GET/POST /task-execution-templates`、`POST /tasks/preflight`、`POST /tasks`、`GET /workflows`、`GET /datasets` | `TaskCreateWizard` 单测覆盖必选校验、执行参数模板填充、Preflight 创建门禁、关键参数变更后重跑要求、阻断风险确认、重跑后清除旧强制创建标记和参数提交；目标前端测试覆盖执行中心先运行 Preflight 后创建、选择执行模板后提交 `execution_template_id`；后端测试覆盖执行模板列表/创建、Preflight 签名字段、过期 Preflight 阻断、伪造 passed Preflight 阻断、`execution_template_id` 快照、`TASK_PREFLIGHT_BLOCKED`、`TASK_PREFLIGHT_STALE`、`allow_blocked_preflight`、`evaluation_goal`、`quality_gate`、`preflight_result`、`execution_config` 落库；Playwright E2E 覆盖真实创建前运行 Preflight |
