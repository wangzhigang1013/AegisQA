...
- `npm test`：46 个前端交互/API client/图模型/任务创建向导/Preflight/首页任务工作台/任务详情驾驶舱/Dataset Lineage/Trace Flow/Trace Tree/Workflow 字段映射/参数预览/报告评测结论/报告根因诊断/Repair Task 生成、工作台、动作闭环、复跑对比、上下文修复建议、二级修复任务与修复树进度/报告风险治理/Experiment/CI Gate/Annotation Queue/Judge 偏差趋势/治理边界测试通过。
...
| 修复任务 | 发起后续动作 | 可用，可从修复任务直接发起人工审核、执行 CI Gate 复测、复跑对比、生成上下文修复建议、拆分二级修复任务、查看修复树进度，并跳转参数治理；动作结果写入 `action_history` 与 `last_action_result`，工作台展示动作历史、最近建议、子任务标记、推荐动作入口和修复进度，避免重复处理同一根因 | `POST /repair-tasks/{repair_task_id}/actions`、`GET /repair-tasks/{repair_task_id}/tree`、`GET /annotation-queue`、`GET /ci-gates/evaluations`、`POST /tasks/{task_id}/attempts` | `tests/test_task_flow_optimization.py` 覆盖 Annotation Queue、CI Gate 复测、复跑创建新 Attempt、RunReport 对比、上下文修复建议、二级修复任务复用和修复树进度聚合；`npm test -- src/test/App.test.tsx -t "修复任务工作台"` 覆盖按钮反馈、动作历史、建议标题、子任务展示、推荐动作和查看进度抽屉 |
