...
| 候选资产中心 | 复跑对比、晋升建议与晋升审批 | 可用，要求候选 Workflow 草稿已发布；后端复用来源 Task 的 Dataset Version 和执行配置创建候选 Task，自动执行并生成候选 Experiment；前端展示 baseline/current/candidate 三方指标、delta、候选任务报告入口和“晋升建议”；晋升建议解释通过率门槛、Badcase 门槛、当前版本改善、baseline 退化和下一步动作；`create_promotion_review` 会渲染为真实按钮，成功后展示 Workflow 晋升审批卡片；hold 候选创建审批会被后端结构化阻断；审批通过后生成 Experiment baseline 替换建议、Workflow 发布记录和 CI Gate 发布评估历史，前端直接展示 Baseline 与 CI Gate 发布资产；baseline 替换建议可应用为当前 baseline，并保留 apply/rollback history | `POST /prompt-skill-candidates/{candidate_id}/retest`、`POST /prompt-skill-candidates/{candidate_id}/promotion-review`、`GET /workflow-promotion-reviews`、`POST /workflow-promotion-reviews/{review_id}/approve|reject`、`GET /experiment-baseline-suggestions`、`GET /experiment-baselines`、`POST /experiment-baseline-suggestions/{suggestion_id}/apply|rollback`、`GET /workflow-release-records`、`GET /ci-gates/evaluations` | `tests/test_task_flow_optimization.py` 覆盖未发布草稿阻断、发布后复跑、同数据集复用、候选 Experiment、三方指标、晋升建议、hold 阻断、晋升审批创建、查询、幂等返回、审批通过、baseline 替换建议、Workflow 发布记录、CI Gate 发布评估历史、baseline 应用、当前 baseline 查询、history、回滚和列表查询；`npm test -- src/test/App.test.tsx -t "候选资产中心"` 覆盖按钮反馈、指标展示、晋升建议、晋升审批创建、审批卡片、通过晋升、发布资产展示、baseline 应用和当前 baseline 展示 |
