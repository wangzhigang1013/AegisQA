...
- 单元/API/扩展测试：`python -m pytest -q` 已通过，覆盖 73 个后端测试点；仍有 Windows `.pytest_cache` 创建警告，不影响结果。
...
- 前端：`npm run typecheck`、`npm test`、`npm run build` 已通过；`npm test` 覆盖 46 个交互/API client/图模型/任务创建向导/Preflight/Run Attempts/Dataset Lineage/Trace Tree/Experiment/CI Gate/Annotation Queue/报告评测结论/Repair Task 生成、工作台、动作闭环、复跑对比、上下文修复建议、二级修复任务与修复树进度/Judge 偏差趋势/治理边界测试；最近一次 Playwright 覆盖 8 条 E2E。
...
| Repair Task 修复闭环 | 已实现基础 | `POST /tasks/{task_id}/repair-tasks/from-diagnostics` 可把诊断 root cause 沉淀为可追踪修复任务；`GET /repair-tasks?source_task_id=` 支持按来源任务查询；`GET /repair-tasks/{repair_task_id}/tree` 返回父任务、子任务、完成率、阻塞子任务和下一步动作；`POST /repair-tasks/{repair_task_id}/start|resolve|reopen` 支持领取、完成、重开并记录审计；`POST /repair-tasks/{repair_task_id}/actions` 支持从修复任务发起 Annotation Queue、CI Gate 复测、`retest_and_compare` 复跑对比、`generate_remediation_plan` 上下文修复建议和 `create_followup_repair_tasks` 二级修复任务，并写入 `action_history` 与 `last_action_result`；复跑动作会创建新 Attempt、自动执行、对比前后 RunReport，并返回 improved / mixed / unchanged / regressed；修复建议会结合复跑状态、诊断根因、弱分层、数据质量和参数治理风险，输出 annotation、dataset、workflow_parameters、retest 等下一步动作；二级任务会保留父任务、推荐动作和目标入口，并重复点击复用已有任务；React 报告中心可一键生成修复任务，`/repair-tasks` 工作台支持筛选、查看证据、领取、完成、重开、发起人工审核、CI Gate 复测、复跑对比、生成修复建议、拆分子任务、查看修复树进度、查看动作历史、最近建议和推荐动作入口，并跳回来源报告、Trace 和参数治理 |
