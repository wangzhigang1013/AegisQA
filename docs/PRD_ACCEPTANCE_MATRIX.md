...
| FR-RP-07 | 报告导出 | 已实现基础 | `export_report_csv`、`export_report_html`，`GET /runs/{run_id}/report/export?file_format=json|csv|html` 保留 Run 级导出；新增 `GET /tasks/{task_id}/report/export?file_format=json|csv|html` 作为产品主入口，导出 Task、Run Report、Badcase 和创建前 Preflight 证据；HTML 导出会转义任务名和 JSON 内容；报告中心前端会将导出内容转成 Blob 并触发浏览器下载，且 HTML/CSV/JSON 三个按钮分别调用对应格式 |
