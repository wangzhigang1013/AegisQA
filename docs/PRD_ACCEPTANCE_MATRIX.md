...
- 单元/API/扩展测试：`python -m pytest -q` 已通过，覆盖 83 个后端测试点；仍有 Windows `.pytest_cache` 创建警告，不影响结果。
