...
- 单元/API/扩展测试：`python -m pytest -q` 已通过，覆盖后端全量测试；仍有 Windows `.pytest_cache` 创建警告，不影响结果。本轮新增 Agent Skill zip 包上传、脚本型运行时、说明型运行时和 Workflow 节点执行回归测试。
...
| CI Gate | 已实现基础页面 | `GET/POST /ci-gates` 支持质量门禁配置保存和列表；`POST /ci-gates/evaluate` 支持按配置和指标阈值 blocking 发布，也支持直接对 Task/Run 抽取指标评估，并保存 `gateeval-*` 历史；`GET /ci-gates/evaluations` 支持按 config、task、run 过滤，无分页参数时保持 legacy 数组响应，带 `page/page_size` 时返回 `{ items, pagination, summary }`，summary 基于筛选后的全量历史计算；React `/ci-gates` 页面支持创建门禁配置、选择 Task/Run 执行评估，并展示阻断原因、实际值、阈值、历史趋势和服务端分页评估历史；Playwright 覆盖真实创建和阻断评估 |
| Annotation Queue | 已实现基础页面 | `POST /annotation-queue/seed-from-run`、`GET /annotation-queue`、分派、review、`POST /annotation-queue/bulk-review`、`GET /annotation-candidates`；队列记录回填来源 Task，支持状态/负责人/来源任务筛选；`GET /annotation-queue?page=&page_size=` 返回服务端分页结果且无分页参数时保持 legacy 数组响应；React `/annotation-queue` 页面支持领取、分派、审核、批量审核、回流 Golden Dataset、候选资产摘要和服务端分页；Playwright 覆盖真实领取、审核、批量审核和回流 |
