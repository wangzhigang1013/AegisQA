...
- 单元/API/扩展测试：`python -m pytest -q` 已通过，覆盖 109 个后端测试点；仍有 Windows `.pytest_cache` 创建警告，不影响结果。
...
| FR-SK-02 | config_schema 生成参数表单 | 已实现基础 | Skill manifest 暴露 `config_schema`；React Skill 详情抽屉展示 schema；Workflow Inspector 会按当前 Skill 的 `config_schema` 渲染参数表单，支持 string/number/integer/boolean/enum/object/array 基础输入，并把配置写入 Workflow 草稿 graph；动态复杂联动表单仍可增强 |
...
| FR-WF-03 | 字段映射与强类型校验 | 已实现 | `resolve_input_mapping`，失败不调用 Skill；Workflow 图校验会读取 Skill `input_schema.required`，在校验/发布阶段阻断必填输入映射缺失并返回 `REQUIRED_INPUT_MAPPING_MISSING`；线性兼容发布入口 `/workflows/publish` 也会校验 Workflow Step 合约，阻断缺少必填映射、空输入路径和空输出路径；Task Preflight 新增 `workflow_schema_mapping` 检查项，可拦截历史坏 Workflow；React Workflow Inspector 字段映射支持可编辑字段名和自定义路径输入，Playwright 覆盖 `row.prompt_text` 保存草稿并从 Workflow 市场重新打开后回放 |
