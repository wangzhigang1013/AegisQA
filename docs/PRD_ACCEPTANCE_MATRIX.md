...
- 单元/API/扩展测试：`python -m pytest -q` 已通过，覆盖 112 个后端测试点；仍有 Windows `.pytest_cache` 创建警告，不影响结果。
...
| FR-WF-03 | 字段映射与强类型校验 | 已实现 | `resolve_input_mapping`，失败不调用 Skill；Workflow 图校验会读取 Skill `input_schema.required`，在校验/发布阶段阻断必填输入映射缺失并返回 `REQUIRED_INPUT_MAPPING_MISSING`；线性兼容发布入口 `/workflows/publish` 也会校验 Workflow Step 合约，阻断缺少必填映射、空输入路径和空输出路径；Task Preflight 新增 `workflow_schema_mapping` 检查项，可拦截历史坏 Workflow；Workflow Graph 发布和 Task Preflight 也会校验 Skill 参数合约，避免参数缺失或类型错误拖到执行期；React Workflow Inspector 字段映射支持可编辑字段名和自定义路径输入，Workflow Console 对字段映射、未知 Skill 和参数门禁错误均展示可操作修复建议，Playwright 覆盖 `row.prompt_text` 保存草稿并从 Workflow 市场重新打开后回放 |
