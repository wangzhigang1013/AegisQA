...
| Task Preflight | 已实现基础 | `POST /tasks/preflight` 在创建任务前检查数据集非空、Workflow 字段映射、Golden 覆盖、Skill 审批状态、质量门槛和成本预算，并持久化返回 `preflight_id`；`GET /task-preflights/{preflight_id}` 可查询用户创建任务前实际看过的预检证据；Preflight 响应返回 `execution_template_id`、`evaluation_goal`、`quality_gate`、`sample_repeat_times`、`cost_budget` 和 `skill_overrides` 关键参数签名；`POST /tasks` 永远基于当前请求重算服务端 Preflight 作为事实源，客户端提交的 `preflight_result` 只用于签名过期判断，不能伪造 passed 绕过字段缺失、Skill 未审批、预算等阻断项；`POST /tasks` 可引用 `preflight_id`，并把它写入 Task `preflight_result` 与 `execution_config`；显式 `preflight_id` 与客户端 `preflight_result.preflight_id` 冲突时返回 `TASK_PREFLIGHT_STALE`；`POST /tasks` 会阻断 `status=blocked` 的服务端 Preflight 并返回 `TASK_PREFLIGHT_BLOCKED`，也会拒绝与当前请求不匹配的旧 `preflight_result` 并返回 `TASK_PREFLIGHT_STALE` 与 mismatch 字段；确需创建坏数据诊断任务时必须显式传 `allow_blocked_preflight=true`，该标记会进入 Task/Run 执行快照和审计；React 任务创建向导要求先运行与当前 Dataset/Workflow/执行模板/质量门槛/repeat/成本预算匹配的 Preflight，关键参数变更后必须重跑，选择执行模板后创建任务会提交 `execution_template_id` 与 `preflight_id`，阻断时默认禁用创建并要求勾选风险确认；任务详情参数页会展示创建前 Preflight ID、状态、摘要和检查项，方便用户复核创建前证据 |
