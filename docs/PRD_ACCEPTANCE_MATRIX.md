...
| FR-AU-04 | 角色权限 | 已实现基础 | `AccessControl`；报告导出区分 `report:read` 与 `report:export`，Evaluator/Reviewer/Admin 可导出，Viewer 只能查看；导出审批使用 `report:export:approve`，当前由 Admin 通过 `*` 权限审批，Reviewer 审批会返回 `REPORT_EXPORT_APPROVAL_FORBIDDEN` |
