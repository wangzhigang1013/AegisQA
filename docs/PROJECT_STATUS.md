...
新一轮“Repair Task Remediation Plan Loop”已完成。本批次把修复任务继续接到“复跑后下一步改哪里”的闭环：`POST /repair-tasks/{repair_task_id}/actions` 新增 `generate_remediation_plan`，会综合复跑对比状态、Task Diagnostics、分层指标、数据质量告警和参数治理风险，生成结构化修复建议。前端修复任务工作台新增“生成建议”按钮和“最近结果”列，用户不用离开根因工单就能看到下一步应该进入人工审核、数据修复、参数治理还是再次复跑。SQLite 轻量仓储仍作为当前推荐本地持久化方案：Task、Run、Workflow、Judge、审计等 JSON 文档可进入 SQLite，Dataset rows、上传文件、Skill 插件包继续保留本地文件路径。
...
- `python -m pytest tests\test_task_flow_optimization.py -q`：6 passed，覆盖 Task Preflight、Repair Task 生成查询、领取/完成/重开状态流转、Repair Task 发起 Annotation Queue / CI Gate 复测、修复后复跑 Attempt 与报告对比，以及上下文修复建议生成。
- `python -m pytest -q`：通过，覆盖 71 个后端测试点；仍有 Windows `.pytest_cache` 创建警告，不影响结果。
...
- `cd frontend && npm test -- src/test/App.test.tsx -t "修复任务工作台"`：4 passed，覆盖修复任务工作台查看证据、领取、完成、发起人工审核、CI Gate 复测、复跑对比和生成修复建议。
- `cd frontend && npm test`：4 个测试文件、44 个测试通过。
...
- 报告、Badcase、Judge 审计和 Repair Task 已经接入基础数据与动作，人工审阅队列、多 Judge 一致性、红队扫描、Judge 偏差趋势、修复任务状态流转、Annotation Queue 发起、CI Gate 复测、修复后复跑对比和上下文修复建议已有最小闭环；后续需要把建议项继续动作化到 Dataset 字段修正、Workflow 参数 diff/回滚和 Prompt/Skill 配置版本对比。
...
- Repair Task 深水区：把当前修复建议继续动作化到 Dataset 字段修复、Workflow 参数 diff/回滚、Prompt/Skill 配置版本对比和自动创建二级修复任务。
