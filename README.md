...
- Task / Run Attempt / Run Item / Step 级执行轨迹
...
后端使用 FastAPI，前端使用 React/Vite/TypeScript。当前产品化入口是 React 工作台，Streamlit 只保留为 legacy demo。
...
AegisQA 的用户主对象是 Task（任务）：一次任务固定绑定 Dataset Version、Workflow Version、执行参数、报告、Badcase、Annotation 和质量门禁结果。

Run 是 Task 的底层执行批次，也可以理解为 Attempt。用户在执行中心主要创建和管理任务；当任务需要重跑时，会创建新的 Run Attempt，旧 Attempt 的报告快照保留，避免历史结果被覆盖。
...
- 执行中心：任务列表、任务详情驾驶舱、Run Attempts、Step Trace、暂停/恢复/取消/重试失败项。
- 报告中心：围绕任务展示指标卡、耗时分布、Badcase 列表、导出入口和分层分析。
- Judge 审计：Profile 审计指标、混淆矩阵、错判样本和偏差分析。
- 治理与审计：RBAC、Skill 生命周期和审计日志；生产适配状态只在部署文档中说明，不作为页面里的可操作状态。
